...
def run_xgb_label(x_train, y_train, x_test, y_test):
    ...
    x_train_dyn.drop(x_train_dyn.columns[[8,9,10]], axis=1, inplace=True)
    x_test_dyn.drop(x_test_dyn.columns[[8,9,10]], axis=1, inplace=True)
    ...
    # xgbr0 = xgb.XGBRegressor(n_estimators=n_est, max_depth=man_d_num)
    # xgbr0.fit(x_train_reg_pin, y_train[0])

    xgbr1 = xgb.XGBRegressor(n_estimators=n_est, max_depth=man_d_num, nthread=20)
    xgbr1.fit(x_train_dyn, y_train[2])
    ...
    y_pre_total = y_pred_reg_pin + y_pred_dyn ## total area


    return y_pred_reg_pin, y_pred_reg_pin2, y_pred_dyn, y_pred_dyn2, y_pre_total
...
def kFold_train(x, y, title):
        ...
        for k, (train, test) in enumerate(kf.split(x,y)):
            ...
            test_all_arr_dyn = np.append(test_all_arr_dyn, np.array(y_test[2], dtype=np.float64))
            pred_all_arr_dyn = np.append(pred_all_arr_dyn, y_pred_dyn)
            train_all_arr_dyn = np.append(train_all_arr_dyn, np.array(y_train[2], dtype=np.float64))
